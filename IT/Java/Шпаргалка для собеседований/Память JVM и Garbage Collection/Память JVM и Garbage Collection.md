...
Тип ссылки определяет, **насколько сильно объект удерживается от сборки мусора**. GC принимает решение об удалении объекта, исходя из самой "сильной" ссылки, которая на него указывает. Все типы (кроме strong) живут в пакете `java.lang.ref` и могут работать в связке с `ReferenceQueue` — очередью, куда GC кладёт ссылку после того, как объект стал недостижим.

```mermaid
flowchart TB
    Obj["Объект в Heap"]
    Strong["Strong Reference<br/>Object o = new Object()"] -->|"никогда не очистится,<br/>пока ссылка жива"| Obj
    Soft["SoftReference"] -->|"очистится только при нехватке памяти"| Obj
    Weak["WeakReference"] -->|"очистится на ближайшем GC,<br/>если нет strong-ссылок"| Obj
    Phantom["PhantomReference"] -->|"объект уже недостижим,<br/>get() всегда null"| Obj
```

| Тип ссылки  | Когда GC удаляет объект                                                                | `get()`           | ReferenceQueue                                      | Применение                                                                                             |
| ----------- | -------------------------------------------------------------------------------------- | ----------------- | --------------------------------------------------- | ------------------------------------------------------------------------------------------------------ |
| **Strong**  | Никогда, пока существует хотя бы одна сильная ссылка (обычное `=`)                     | —                 | —                                                   | Обычные ссылки. Именно они держат объекты живыми и являются причиной утечек памяти                     |
| **Soft**    | Только когда JVM **не хватает памяти** (перед `OutOfMemoryError`)                      | объект или `null` | опционально, после очистки                          | Кэши, которые не жалко потерять под давлением памяти (memory-sensitive cache)                          |
| **Weak**    | На **ближайшем** цикле GC, если на объект больше нет strong/soft ссылок                | объект или `null` | опционально, после очистки                          | `WeakHashMap`, метаданные/кэши, привязанные к жизни ключа; не мешает сборке                            |
| **Phantom** | Объект уже признан недостижимым; ссылка нужна для пост-обработки **после** финализации | **всегда `null`** | обязательна (без неё PhantomReference бессмысленна) | Замена устаревшему `finalize()`: освобождение native-ресурсов, надёжная очистка после удаления объекта |

> **Иерархия силы:** Strong → Soft → Weak → Phantom. Чем "слабее" ссылка, тем раньше GC готов забрать объект.

**Достижимость (reachability)** — объект имеет статус по самой сильной ссылке на него:
- *strongly reachable* — есть путь от GC roots только через strong-ссылки;
- *softly reachable* — нет strong, но есть soft;
- *weakly reachable* — нет strong/soft, но есть weak;
- *phantom reachable* — недостижим, финализирован, но есть phantom-ссылка.

**ReferenceQueue** — механизм уведомления: когда GC очищает Soft/Weak/Phantom ссылку (объект стал недостижим), сама ссылка помещается в зарегистрированную очередь. Приложение опрашивает очередь (`queue.poll()` / `remove()`) и выполняет cleanup. Для PhantomReference это единственный способ узнать о "смерти" объекта, т.к. `get()` всегда возвращает `null`.

> **Почему Phantom вместо `finalize()`:** `finalize()` непредсказуем по времени, может воскресить объект (создать на него strong-ссылку) и тормозит сборку. PhantomReference + ReferenceQueue даёт детерминированную точку очистки уже после того, как объект гарантированно недостижим и не может быть воскрешён. Именно так устроен `java.lang.ref.Cleaner` (Java 9+).